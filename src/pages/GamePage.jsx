...
export default function GamePage({ project, profileName, currentHeroTheme, onBack }) {
  const gameDescription = project.longDescription || project.description || project.shortDescription || ''
  const hasItch = Boolean(project.itchLink)
  const hasTrailer = Boolean(project.trailer)
  const hasExtraNotes = Boolean(project.expectations || project.learnings)
